...
class SCROLLCompilerPluginConfig() {
  ...
  private def getInstanceName(of: EObject): String =
    of.eClass().getEAllAttributes.asScala.find(_.getName == "name") match {
  ...
  private def collectRoles(of: EObject): List[EObject] = of
    .eContents()
    .asScala
    .toList
    .flatMap(e =>
  ...
  private def constructRel[RST >: Null <: AnyRef, RT >: Null <: AnyRef](elem: EObject): (RST, List[RT]) = {
    ...
    val rsts    = roles
      .filter { role =>
        val incoming =
          role.asInstanceOf[DynamicEObjectImpl].dynamicGet(1).asInstanceOf[EcoreEList[DynamicEObjectImpl]].asScala
        val inCond = incoming match {
          case null => false
          case _    => incoming.exists(e => e.dynamicGet(0).asInstanceOf[String] == rstName)
        }
        val outgoing =
          role.asInstanceOf[DynamicEObjectImpl].dynamicGet(2).asInstanceOf[EcoreEList[DynamicEObjectImpl]].asScala
  ...
  private def construct[NT >: Null <: AnyRef, RT >: Null <: AnyRef, CT >: Null <: AnyRef, RST >: Null <: AnyRef]()
    ...
    loadModel().getAllContents.asScala
